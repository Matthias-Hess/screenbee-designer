...
function deriveSwitchTable(project, hasRuleFor) {
  ...
  for (const screen of project.screens || []) {
    for (const obj of collectObjects(screen.objects)) {
      ...
      // A read topic may be a "<topic>#<path>" composite naming one field of
      // a JSON payload. Deriving from it would answer on the composite string
      // itself - not a topic anyone can publish to ("#" is a reserved
      // wildcard) and not what getTopicValue() looks up either. Answering on
      // the bare topic instead is no better: the engine knows one field's
      // value, not the whole payload, so it would have to invent the rest.
      //
      // So it declines, and says so - unless the project already declares the
      // round trip itself, which is the supported way to describe this and
      // what "Camper Licht"'s Lock screen does. Silence here would be the bad
      // kind: hil:mock would look like it had covered the switch while
      // publishing to a topic no broker accepts (2026-08-26).
      if (readTopic.includes("#")) {
        const described = (props.states || []).some((s) => s.writeValue && hasRuleFor(writeTopic, s.writeValue))
        if (!described) {
          skipped.push(
            `${obj.id}: reads a JSON field (${readTopic}) - a round trip through one field cannot be derived, ` +
              `declare it as a mock rule on ${writeTopic}`,
          )
        }
        continue
      }
