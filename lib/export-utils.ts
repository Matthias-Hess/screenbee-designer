...
export class ExportManager {
  static exportToESP32(project: ScreenmanProject): ESP32Export {
    ...
    const totalFonts = (project.fonts || []).length
...
int ${className}::getScreenCount() {
  ...
  private static generateArduinoReadme(project: ScreenmanProject): string {
    ...
    const fontList = (project.fonts || []).length > 0 
      ? (project.fonts || []).map(font => `- **${font.name}**: ${font.size}px (baseline offset: ${font.baselineOffset}px)`).join('\n')
...
## Font Usage
Font baseline offsets are included in the export data for accurate text positioning.
Use the \`baselineOffset\` values from the fonts array to position text correctly on your display.
