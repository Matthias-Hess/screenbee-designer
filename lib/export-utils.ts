...
export class ExportManager {
  static exportToESP32(project: ScreenmanProject): ESP32Export {
    ...
    return {
      ...
      screens: project.screens.map((screen) => ({
        ...
        backgroundImageAssetId: screen.backgroundImageAssetId, // Include background asset reference in export
      })),
  ...
  private static sanitizePropertiesForESP32(obj: ScreenmanObject): Record<string, any> {
    ...
    Object.entries(obj.properties).forEach(([key, value]) => {
      if (key === "topicId" && typeof value === "string") {
        // Note: In a real implementation, you'd need access to the project topics here
        // For now, we'll store the topicId and let the Arduino code resolve it
        sanitized["topicId"] = value
      }
...
## Screen Dimensions
${project.screens.map((screen, index) => `- **${screen.name}**: ${screen.width} × ${screen.height} pixels`).join("\n")}
